  ...
  useEffect(() => {
    const checkBooking = async () => {
      try {
        ...
        const hasBooking = bookings.some(booking => booking.room === room._id)
        setCanReview(hasBooking)
      } catch (error) {
        // If 404, just set canReview to false (user not logged in or endpoint missing)
        if (error.response && error.response.status === 404) {
          setCanReview(false)
          // Do not log 404 errors to avoid console spam
        } else {
          // Only log other errors once per session
          if (!window.__roomCardBookingErrorLogged) {
            console.error('Error checking bookings:', error)
            window.__roomCardBookingErrorLogged = true
          }
        }
      }
    }
